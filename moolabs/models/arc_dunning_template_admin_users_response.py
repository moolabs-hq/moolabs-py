...
class ArcDunningTemplateAdminUsersResponse(BaseModel):
    """
    ...
    users: List[ArcDunningTemplateAdminUserDto]
    __properties: ClassVar[List[str]] = ["tenant_id", "org_id", "permission", "can_manage", "users"]

    @field_validator('permission')
    def permission_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['arc_dunning_template_admin']):
            raise ValueError("must be one of enum values ('arc_dunning_template_admin')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "users": [ArcDunningTemplateAdminUserDto.from_dict(_item) for _item in obj["users"]] if obj.get("users") is not None else None
        })
        return _obj
