...
class PackProvenance(BaseModel):
    ...
    """ # noqa: E501
    authored_by: Optional[StrictStr] = 'moolabs'
    forked_from: Optional[StrictStr] = None
    signoffs: Optional[List[Dict[str, Any]]] = None
    __properties: ClassVar[List[str]] = ["authored_by", "forked_from", "signoffs"]

    @field_validator('authored_by')
    def authored_by_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['moolabs', 'tenant']):
            raise ValueError("must be one of enum values ('moolabs', 'tenant')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "authored_by": obj.get("authored_by") if obj.get("authored_by") is not None else 'moolabs',
