# coding: utf-8

"""
    Moolabs API

    Unified Moolabs SDK. Capability-based public surface stitched from internal service specs: bff, meter, arc, acute. Customers see capability namespaces (usage, billing, collections, etc.); the SDK routes per-operation internally.

    The version of the OpenAPI document: 0.2.0-rc10
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from moolabs.models.credit_memo_list_response import CreditMemoListResponse

class TestCreditMemoListResponse(unittest.TestCase):
    """CreditMemoListResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> CreditMemoListResponse:
        """Test CreditMemoListResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `CreditMemoListResponse`
        """
        model = CreditMemoListResponse()
        if include_optional:
            return CreditMemoListResponse(
                items = [
                    moolabs.models.credit_memo_response.CreditMemoResponse(
                        id = '', 
                        tenant_id = '', 
                        account_id = '', 
                        case_id = '', 
                        reference_number = '', 
                        reason = '', 
                        status = '', 
                        credit_amount_micros = 56, 
                        applied_amount_micros = 56, 
                        unapplied_amount_micros = 56, 
                        currency_code = '', 
                        subsidiary = '', 
                        description = '', 
                        created_by = '', 
                        approved_by = '', 
                        approved_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        closed_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), )
                    ],
                total = 56,
                page = 56,
                page_size = 56
            )
        else:
            return CreditMemoListResponse(
                items = [
                    moolabs.models.credit_memo_response.CreditMemoResponse(
                        id = '', 
                        tenant_id = '', 
                        account_id = '', 
                        case_id = '', 
                        reference_number = '', 
                        reason = '', 
                        status = '', 
                        credit_amount_micros = 56, 
                        applied_amount_micros = 56, 
                        unapplied_amount_micros = 56, 
                        currency_code = '', 
                        subsidiary = '', 
                        description = '', 
                        created_by = '', 
                        approved_by = '', 
                        approved_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        closed_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        created_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                        updated_at = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), )
                    ],
                total = 56,
                page = 56,
                page_size = 56,
        )
        """

    def testCreditMemoListResponse(self):
        """Test CreditMemoListResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
