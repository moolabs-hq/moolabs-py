...
class Addon(BaseModel):
    ...
    version: Annotated[int, Field(strict=True, ge=1)] = Field(description="Version of the add-on. Incremented when the add-on is updated.")
    ...
    currency: Annotated[str, Field(min_length=3, strict=True, max_length=3)] = Field(description="The currency code of the add-on.")
    effective_from: Optional[datetime] = Field(default=None, description="The date and time when the add-on becomes effective. When not specified, the add-on is a draft.", alias="effectiveFrom")
    effective_to: Optional[datetime] = Field(default=None, description="The date and time when the add-on is no longer effective. When not specified, the add-on is effective indefinitely.", alias="effectiveTo")
    status: AddonStatus = Field(description="The status of the add-on. Computed based on the effective start and end dates: - draft = no effectiveFrom - active = effectiveFrom <= now < effectiveTo - archived  = effectiveTo <= now")
    ...
    __properties: ClassVar[List[str]] = ["id", "name", "description", "metadata", "createdAt", "updatedAt", "deletedAt", "key", "annotations", "version", "instanceType", "currency", "effectiveFrom", "effectiveTo", "status", "rateCards", "validationErrors"]
