...
class ProviderConfirmationRepairRequest(BaseModel):
    """
    ...
    external_message_id: Optional[Annotated[str, Field(strict=True, max_length=255)]] = None
    __properties: ClassVar[List[str]] = ["outcome", "note", "external_message_id"]

    @field_validator('outcome')
    def outcome_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['sent', 'not_sent']):
            raise ValueError("must be one of enum values ('sent', 'not_sent')")
        return value
