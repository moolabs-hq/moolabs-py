...
class LifecycleEventRequest(BaseModel):
    ...
    event_type: StrictStr = Field(description="Event type: ACTIVATED, UPDATED, CANCELLED, PAUSED, RESUMED")
    event_id: StrictStr = Field(description="Unique event ID (for idempotency)")
    effective_at: StrictStr = Field(description="When the event takes effect (ISO 8601)")
    subscription_data: Optional[Dict[str, Any]] = Field(default=None, description="Optional subscription data")
    payload_json: Optional[StrictStr] = Field(default=None, description="Optional JSON payload")
    __properties: ClassVar[List[str]] = ["subscription_id", "event_type", "event_id", "effective_at", "subscription_data", "payload_json"]
